...
The current generator is recipe-driven rather than an unrestricted AI skill author. `generator.js` contains the safe vocabulary of names, effect stats, targets, and progression formulas; the synthesizer freely combines observed patterns with that vocabulary and the game applies the resulting numeric modifiers generically. To make generation fully production-ready, move the vocabularies into a JSON/config registry and add a validator that checks buff bounds, incompatible modifiers, target stat, and deterministic replay. It can then vary names, effects, formulas, and sequences freely within the registered mechanics without executing arbitrary generated code.
